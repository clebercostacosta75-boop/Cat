...
Deno.serve(async (req) => {
  ...
  const defaults = {
    ...
    certificacao: ['dashboard_certificacao','certificacoes','alertas_vencimento','designer_certificados','assinaturas_digitais','auditoria_certificados'],
    atendimento: ['dashboard','dashboard_comercial','gestao_academica_individual','agenda_treinamentos'],
    instrutor: ['dashboard_instrutor','agenda_treinamentos','chamada_presencial'], auditor: ['dashboard','auditoria_certificados','log_auditoria','auditoria_completa','log_acesso','relatorios'],
    editor: ['dashboard'], cliente: ['dashboard'], aluno: [], empresa: [], personalizado: []
  };
  try {
    ...
    for (const user of users) {
      ...
      if (!profile) {
        const newRole = user.role === 'admin' ? 'admin' : (roleAliases[user.custom_role] || 'cliente');
        const newPermissions = defaults[newRole] || [];
        if (!dryRun) profile = await base44.asServiceRole.entities.UserProfile.create({ user_id: user.id, user_email: email, user_name: user.full_name || email, role: newRole, permissions: newPermissions, company_permissions: user.company_permissions || [], status: 'active' });
        report.created.push({ user_id: user.id, profile_id: profile?.id || null, email, role: newRole, permissions: newPermissions });
        continue;
      }
      if (profile.user_id && profile.user_id !== user.id) { report.errors.push({ code: 'profile_mismatch', profile_id: profile.id, email }); continue; }
      const canonicalRole = roleAliases[profile.role] || profile.role || 'cliente';
      const currentPermissions = normalizePermissions(profile.permissions);
      const normalized = currentPermissions.length ? currentPermissions : (defaults[canonicalRole] || []);
      const changes = {};
      if (canonicalRole !== profile.role) changes.role = canonicalRole;
