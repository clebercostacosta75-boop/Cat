...
const FunctionDisplay = ({ toolCall }) => {
    ...
    const statusConfig = {
        ...
        completed: isError ? 
            { icon: AlertCircle, color: 'text-red-500', text: 'Falha' } : 
            { icon: CheckCircle2, color: 'text-green-600', text: 'Concluído' },
        success: { icon: CheckCircle2, color: 'text-green-600', text: 'Concluído' },
        failed: { icon: AlertCircle, color: 'text-red-500', text: 'Falha' },
        error: { icon: AlertCircle, color: 'text-red-500', text: 'Falha' }
...
export default function MessageBubble({ message }) {
    ...
    return (
        <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
            {!isUser && (
                <div className="h-7 w-7 rounded-lg bg-gradient-to-br from-purple-100 to-blue-100 flex items-center justify-center mt-0.5">
            ...
            <div className={cn("max-w-[85%]", isUser && "flex flex-col items-end")}>
                {message.content && (
                    <div className={cn(
                        ...
                        isUser ? "bg-gradient-to-br from-purple-600 to-blue-600 text-white" : "bg-white border border-slate-200"
                    )}>
                        {isUser ? (
