...
import { executarInscricaoIndividual } from "@/lib/inscricaoIndividual";
import InscricaoConcluidaStep from "@/components/vendas/InscricaoConcluidaStep";
...
export default function MatriculaRapidaModal({ open, onClose, onSuccess }) {
  ...
  const [step, setStep] = useState("cpf"); // cpf | confirmar_existente | form | done
  const [cpf, setCpf] = useState("");
  const [aluno, setAluno] = useState(null);
  const [novoAluno, setNovoAluno] = useState(EMPTY_ALUNO);
  const [tipo, setTipo] = useState("oferta"); // oferta | pacote | curso
  const [ofertaId, setOfertaId] = useState("");
  const [pacoteId, setPacoteId] = useState("");
  const [cursoId, setCursoId] = useState("");
  const [cursoAvulso, setCursoAvulso] = useState({ data_inicio: "", data_termino: "", valor: "" });
  ...
  const { data: courses = [] } = useQuery({ queryKey: ["courses"], queryFn: () => base44.entities.Course.list("-name", 200), enabled: open });
  ...
  useEffect(() => {
    if (open) {
      ...
      setTipo("oferta"); setOfertaId(""); setPacoteId(""); setCursoId("");
      setCursoAvulso({ data_inicio: "", data_termino: "", valor: "" });
      setDatasPacote([]); setPgto(EMPTY_PGTO); setResultado(null);
    }
  }, [open]);
  ...
  const handleVerificarCpf = async () => {
    if (!validarCPF(cpf)) {
      ...
      await logVenda("update", { entity_type: "Student", entity_name: formatarCPF(cpf), details: "FASE 4 Inscrição: tentativa de inscrição BLOQUEADA — CPF inválido" });
      return;
    }
    ...
    if (existente) {
      ...
      await logVenda("view", { entity_type: "Student", entity_id: existente.id, entity_name: existente.full_name, details: "FASE 4 Inscrição: inscrição iniciada — CPF já cadastrado, cadastro existente será reutilizado (duplicidade evitada)" });
    } else {
      setAluno(null);
  ...
  const handleConfirmar = async () => {
    ...
    if (tipo === "curso") {
      ...
      cursosMatricula = [{ course_id: item.course_id, course_name: item.course_name, carga_horaria: item.carga_horaria, data_inicio: item.data_inicio, data_termino: item.data_termino }];
    } else {
      item = pacote;
      ...
      for (const c of datasPacote) {
        if (!c.data_inicio || !c.data_termino) { toast.error(`Não é possível concluir a inscrição sem definir o período de realização do curso "${c.course_name}".`); return; }
        if (c.data_inicio > c.data_termino) { toast.error(`Curso "${c.course_name}": data de início posterior ao término.`); return; }
      }
      cursosMatricula = datasPacote;
    }
    ...
    setSaving(true);
    try {
      const res = await executarInscricaoIndividual({
        user, atendenteNome,
        alunoExistente: aluno, novoAluno, cpf,
        tipo, item, cursosMatricula,
        pgto, origemInscricao: pgto.origem_inscricao,
        valorAvulso: cursoAvulso.valor,
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Zap className="w-5 h-5 text-amber-500" /> Nova Inscrição Individual</DialogTitle>
        </DialogHeader>

        {/* Passo 1: CPF */}
        {step === "cpf" && (
          <div className="space-y-3">
            <p className="text-sm text-gray-500">Informe o CPF do aluno. O sistema verifica automaticamente se ele já está cadastrado — sem duplicidade.</p>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button className="bg-gray-900 hover:bg-gray-800" onClick={() => setStep("form")}>
                <CheckCircle className="w-4 h-4 mr-1" /> Sim, nova inscrição
              </Button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <div className="border rounded-md p-3 space-y-2">
              <div className="flex gap-1 bg-gray-100 rounded-lg p-1 w-fit">
                ...
                <button onClick={() => setTipo("curso")} className={`px-3 py-1.5 rounded-md text-xs font-medium flex items-center gap-1 ${tipo === "curso" ? "bg-white shadow-sm text-gray-900" : "text-gray-500"}`}><GraduationCap className="w-3.5 h-3.5" /> Curso Avulso</button>
              </div>
                ...
                <>
                  ...
                  {oferta && (
                    <div className="text-xs text-gray-600 bg-gray-50 rounded-md p-2 space-y-0.5">
                      <p>📅 Período (alimenta a Certificação): <strong>{oferta.data_inicio || "—"} → {oferta.data_termino || "—"}</strong> • ⏱ {oferta.carga_horaria || "—"}</p>
                      <p>📍 {oferta.local || "—"} • {oferta.modalidade || "—"} • 🎟 Vagas: {vagasDisp(oferta)} de {oferta.vagas_total} • Status: {oferta.status}</p>
              ...
              {tipo === "curso" && (
                <div className="space-y-2">
                  <Select value={cursoId} onValueChange={setCursoId}>
                    <SelectTrigger><SelectValue placeholder="Selecione o curso avulso..." /></SelectTrigger>
                    <SelectContent>
                      {courses.map(c => <SelectItem key={c.id} value={c.id}>{c.name}{c.carga_horaria ? ` — ${c.carga_horaria}` : ""}</SelectItem>)}
                    </SelectContent>
                  </Select>
                  {curso && (
                  ...
                  {curso && <p className="text-xs text-gray-500">📅 O período informado alimenta a matrícula e será usado pela Certificação.</p>}
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                ...
                <div className="col-span-2">
                  ...
                  <Select value={pgto.forma_pagamento} onValueChange={v => setPgto(p => ({ ...p, forma_pagamento: v }))}>
                    ...
                    <SelectContent>
                      {(formasAceitas.length ? formasAceitas : ["Pix", "Boleto", "Cartão de Crédito", "Cartão de Débito", "Dinheiro", "Transferência Bancária", "Cortesia"]).map(f => <SelectItem key={f} value={f}>{f}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div>
        ...
        {/* Passo 3: automação concluída */}
        {step === "done" && resultado && (
          <InscricaoConcluidaStep resultado={resultado} onClose={onClose} />
        )}
      </DialogContent>
    </Dialog>
  );
