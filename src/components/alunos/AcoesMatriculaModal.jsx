...
import { RefreshCw, XCircle, UserPlus, Edit, AlertTriangle, CheckCircle, Shield } from "lucide-react";
...
export default function AcoesMatriculaModal({ enrollment, onClose, onDone }) {
  ...
  const audit = (action, details) => base44.entities.AuditLog.create({
  ...
  const encerrarContrato = async (motivo) => {
    ...
    await base44.entities.Contract.update(c.id, {
      ...
      notes: [c.notes, `${AUDIT}: ${assinado ? "substituído por aditivo (contrato assinado não é alterado silenciosamente)" : "cancelado"} — ${motivo}`].filter(Boolean).join(" | "),
    });
    await audit("update", `contrato ${c.contract_number} ${assinado ? "marcado como Substituído por Aditivo — nova assinatura será exigida" : "cancelado"} — ${motivo}`);
    return c;
  };

  const gerarNovoContrato = async (studentId, enrollmentId) => {
    try {
      const res = await base44.functions.invoke("gerarContrato", { student_id: studentId, enrollment_id: enrollmentId, skip_financeiro: true, force_regen: true });
      if (res.data?.contract_number) await audit("create", `novo contrato ${res.data.contract_number} gerado automaticamente`);
      return res.data;
    } catch {
      await audit("update", "novo contrato NÃO pôde ser gerado — pendência registrada, regularizar dados e gerar na aba Contratos");
      return null;
    }
  };
  ...
  const liberarVaga = async () => {
    ...
    await audit("update", `vaga liberada em "${item.nome_comercial || item.nome}"`);
  };
  ...
  const handleCancelar = async () => {
    await base44.entities.StudentCourseEnrollment.update(enrollment.id, {
      ...
      notes: [enrollment.notes, `Inscrição CANCELADA em ${new Date().toLocaleString("pt-BR")}. Justificativa: ${justificativa}`].filter(Boolean).join(" | "),
    });
    const lancs = await lancamentosMatricula();
    for (const l of lancs) {
      if (["Pago", "Recebido"].includes(l.status)) {
        await base44.entities.LancamentoFinanceiro.update(l.id, { observacoes: [l.observacoes, `${AUDIT}: matrícula cancelada com pagamento realizado — histórico mantido, avaliar necessidade de reembolso. Justificativa: ${justificativa}`].filter(Boolean).join(" | ") });
        await audit("update", "financeiro já pago mantido no histórico — marcado para avaliação de reembolso");
      } else if (l.status !== "Cancelado") {
        await base44.entities.LancamentoFinanceiro.update(l.id, { status: "Cancelado", natureza: "Cancelado", observacoes: [l.observacoes, `${AUDIT}: cobrança cancelada junto com a inscrição. Justificativa: ${justificativa}`].filter(Boolean).join(" | ") });
        await audit("update", "cobrança pendente cancelada");
      }
    }
    await liberarVaga();
    await encerrarContrato(`inscrição cancelada — ${justificativa}`);
    await audit("update", `inscrição CANCELADA. Justificativa: ${justificativa}. Matrícula preservada como histórico (não excluída).`);
    return true;
  };
  ...
  const handleTrocar = async () => {
    ...
    if (!novoCurso.data_inicio || !novoCurso.data_termino) { toast.error("Não é possível concluir a troca sem definir o período de realização do curso."); return false; }
    ...
    const nova = await base44.entities.StudentCourseEnrollment.create({
      ...
      notes: `Criada por TROCA DE CURSO (anterior: ${enrollment.course_name}, matrícula ${enrollment.id}). Justificativa: ${justificativa}`,
    });
    await base44.entities.StudentCourseEnrollment.update(enrollment.id, {
      status_matricula: "Cancelado",
    ...
    for (const l of lancs) {
      if (!["Pago", "Recebido", "Cancelado"].includes(l.status)) {
        await base44.entities.LancamentoFinanceiro.update(l.id, {
          ...
          observacoes: [l.observacoes, `${AUDIT}: financeiro recalculado por troca de curso. Valor anterior: R$ ${valorAnterior.toFixed(2)} → novo: R$ ${novoValor.toFixed(2)}. Justificativa: ${justificativa}`].filter(Boolean).join(" | "),
        });
        await audit("update", `financeiro recalculado: R$ ${valorAnterior.toFixed(2)} → R$ ${novoValor.toFixed(2)}`);
      }
    }
    await encerrarContrato(`troca de curso para ${c.name} — ${justificativa}`);
    await gerarNovoContrato(enrollment.student_id, nova.id);
    await audit("update", `TROCA DE CURSO: "${enrollment.course_name}" → "${c.name}". Novo período: ${novoCurso.data_inicio} → ${novoCurso.data_termino}. Valor anterior: R$ ${valorAnterior.toFixed(2)} | novo: R$ ${novoValor.toFixed(2)}. Justificativa: ${justificativa}`);
    return true;
  };

  // ── TRANSFERIR ───────────────────────────────────────────────────────────
  const verificarCpfNovo = async () => {
    if (!validarCPF(cpfNovo)) { toast.error("CPF do novo aluno inválido."); return; }
  ...
  const handleTransferir = async () => {
    if (!cpfVerificado) { toast.error("Verifique o CPF do novo aluno primeiro."); return false; }
    ...
    if (!novo) {
      ...
      await audit("create", `novo aluno criado para transferência: ${novo.full_name} (CPF ${novo.cpf})`);
    }
    const agora = new Date().toISOString();
    const nova = await base44.entities.StudentCourseEnrollment.create({
    ...
    await base44.entities.StudentCourseEnrollment.update(enrollment.id, {
      status_matricula: "Cancelado",
      notes: [enrollment.notes, `TRANSFERIDA para ${novo.full_name} (nova matrícula ${nova.id}). Justificativa: ${justificativa}`].filter(Boolean).join(" | "),
    });
    const lancs = await lancamentosMatricula();
    for (const l of lancs) {
      if (!["Pago", "Recebido", "Cancelado"].includes(l.status)) {
        ...
        await audit("update", `financeiro transferido para ${novo.full_name}`);
      }
    }
    ...
    await audit("update", `TRANSFERÊNCIA DE INSCRIÇÃO: ${enrollment.student_name} → ${novo.full_name} (CPF ${formatarCPF(cpfNovo)}). Curso: ${enrollment.course_name}. Histórico completo preservado. Justificativa: ${justificativa}`);
    return true;
  };
  ...
  const handleAlterar = async () => {
    ...
    if (String(alt.unit_value) !== String(enrollment.unit_value ?? "")) { upd.unit_value = parseFloat(alt.unit_value) || 0; mudancas.push(`valor: R$ ${enrollment.unit_value || 0} → R$ ${upd.unit_value}`); }
    if (String(alt.desconto) !== String(enrollment.desconto ?? 0)) { upd.desconto = parseFloat(alt.desconto) || 0; mudancas.push(`desconto: R$ ${enrollment.desconto || 0} → R$ ${upd.desconto}`); }
    if (alt.forma_pagamento !== (enrollment.forma_pagamento || "")) { upd.forma_pagamento = alt.forma_pagamento; mudancas.push(`forma de pagamento: ${enrollment.forma_pagamento || "—"} → ${alt.forma_pagamento}`); }
    ...
    if (!alt.start_date || !alt.end_date) { toast.error("Não é possível remover o período de realização do curso."); return false; }
    ...
    if (upd.unit_value !== undefined || upd.data_vencimento_pagamento !== undefined) {
      const lancs = await lancamentosMatricula();
      for (const l of lancs) {
        if (!["Pago", "Recebido", "Cancelado"].includes(l.status)) {
          await base44.entities.LancamentoFinanceiro.update(l.id, {
            ...(upd.unit_value !== undefined ? { valor: Math.max(0, upd.unit_value - (upd.desconto ?? enrollment.desconto ?? 0)) } : {}),
            ...(upd.data_vencimento_pagamento ? { data_vencimento: upd.data_vencimento_pagamento } : {}),
            observacoes: [l.observacoes, `${AUDIT}: financeiro atualizado — ${mudancas.join("; ")}. Justificativa: ${justificativa}`].filter(Boolean).join(" | "),
          });
          await audit("update", "financeiro pendente atualizado conforme alteração da matrícula");
        }
      }
    }
    ...
    await audit("update", `ALTERAÇÃO CONTROLADA: ${mudancas.join("; ")}. Justificativa: ${justificativa}`);
    return true;
  };
  ...
  const handleExecutar = async () => {
    ...
    if ((modo === "trocar" || modo === "transferir") && temCertificado) {
      ...
      await audit("update", `tentativa de ${modo === "trocar" ? "troca de curso" : "transferência"} BLOQUEADA — certificado já emitido (exige fluxo de reemissão/revogação)`);
      return;
    }
    setSaving(true);
    try {
      ...
      if (ok) {
        toast.success("Ação executada e registrada com histórico completo na auditoria.");
        onDone?.();
        onClose();
      }
    } catch (e) {
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Shield className="w-5 h-5 text-gray-700" /> Ações da Inscrição — {enrollment.student_name}</DialogTitle>
        </DialogHeader>

        <div className="p-2.5 bg-gray-50 border rounded-md text-sm">
          <p className="font-medium text-gray-800">{enrollment.course_name}</p>
          <p className="text-xs text-gray-500">{enrollment.start_date} → {enrollment.end_date} • {enrollment.status_matricula || "Matriculado"} • 💰 {enrollment.status_pagamento || "-"}</p>
        </div>

        {jaCancelada && (
          <div className="p-2.5 bg-red-50 border border-red-200 rounded-md text-xs text-red-700 flex items-center gap-2">
            <XCircle className="w-4 h-4 flex-shrink-0" /> Esta inscrição já está cancelada. O histórico é preservado.
          </div>
        )}
        ...
        {!jaCancelada && (
          <>
            <div className="grid grid-cols-2 gap-2">
              {MODOS.map(m => (
                <Button key={m.key} variant="outline" size="sm"
                  className={`justify-start text-xs h-9 ${m.color} ${modo === m.key ? "ring-2 ring-offset-1 ring-gray-400 bg-gray-50" : ""}`}
            ...
            {temCertificado && (modo === "trocar" || modo === "transferir") && (
              <div className="p-2.5 bg-red-50 border border-red-200 rounded-md text-xs text-red-700 flex items-start gap-2">
                ...
                Esta matrícula já possui certificado emitido. A troca/transferência simples é bloqueada — utilize o fluxo de reemissão/revogação da Certificação.
              </div>
            )}

            {modo === "trocar" && (
              <div className="border rounded-md p-3 space-y-2">
                <p className="text-xs font-semibold text-gray-500 uppercase">Novo curso</p>
                ...
                <p className="text-xs text-gray-500">A matrícula atual será cancelada com histórico, uma nova será criada, o financeiro pendente recalculado e um novo contrato gerado.</p>
              </div>
            )}

            {modo === "alterar" && (
              <div className="border rounded-md p-3 space-y-2">
                <p className="text-xs font-semibold text-gray-500 uppercase">Alterar dados</p>
                <div className="grid grid-cols-2 gap-2">
                ...
                <p className="text-xs text-gray-500">Se houver contrato assinado, ele será marcado como substituído por aditivo e um novo será gerado (nada é alterado silenciosamente).</p>
              </div>
            )}

            {modo === "cancelar" && (
              <div className="p-3 bg-red-50 border border-red-200 rounded-md text-xs text-red-700 space-y-1">
                <p className="font-semibold">O cancelamento irá:</p>
                <p>• Marcar a matrícula como Cancelada (sem excluir histórico)</p>
                <p>• Cancelar cobrança pendente (pagamentos realizados são mantidos e marcados para avaliação de reembolso)</p>
                <p>• Liberar a vaga da oferta/pacote, se houver</p>
                <p>• Cancelar o contrato vinculado</p>
              </div>
            )}

            {modo === "transferir" && (
              <div className="border rounded-md p-3 space-y-2">
                <div className="p-2 bg-cyan-50 border border-cyan-200 rounded text-xs text-cyan-800 flex items-start gap-1.5">
                  <AlertTriangle className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
                  Esta ação transferirá a inscrição para outro aluno e será registrada com histórico completo.
                </div>
                <Label className="text-xs">CPF do novo aluno *</Label>
                <div className="flex gap-2">
                  <Input value={cpfNovo} onChange={e => { setCpfNovo(formatarCPF(e.target.value)); setCpfVerificado(false); setNovoAlunoTransf(null); }} placeholder="000.000.000-00" maxLength={14} />
                  <Button size="sm" variant="outline" onClick={verificarCpfNovo}>Verificar</Button>
                </div>
                {cpfVerificado && novoAlunoTransf && (
                  <div className="p-2 bg-emerald-50 border border-emerald-200 rounded text-xs text-emerald-800 flex items-center gap-1.5">
                    <CheckCircle className="w-3.5 h-3.5" /> Aluno encontrado: <strong>{novoAlunoTransf.full_name}</strong>
                  </div>
                )}
                {cpfVerificado && !novoAlunoTransf && (
                  <div className="space-y-2">
                    <p className="text-xs text-amber-700">CPF não cadastrado — informe os dados básicos do novo aluno:</p>
            ...
            {modo && (
              <div>
                ...
                <Textarea rows={2} value={justificativa} onChange={e => setJustificativa(e.target.value)} placeholder="Descreva o motivo desta ação (registrado na auditoria)..." />
              </div>
            )}
            ...
            <div className="flex justify-end gap-2 pt-2 border-t">
              ...
              {modo && (
                <Button className="bg-gray-900 hover:bg-gray-800" onClick={handleExecutar} disabled={saving}>
                  {saving ? "Executando..." : <><CheckCircle className="w-4 h-4 mr-1" /> Executar Ação</>}
                </Button>
              )}
            </div>
          </>
