...
import { Plus, Search, FileText, Edit2, Trash2, AlertTriangle, CheckCircle, Clock } from "lucide-react";
...
const EMPTY = {
  ...
  crea_cau: "", data_elaboracao: "", data_revisao: "", data_validade: "",
  versao: "1.0", status: "Elaboração", grau_risco: 2, observacoes: ""
};
...
export default function PGRTab() {
  ...
  const load = async () => {
    setLoading(true);
    const data = await base44.entities.PGR.list("-created_date", 100);
    setRecords(data);
    setLoading(false);
  };
  ...
  const save = async () => {
    setSaving(true);
    if (editing) await base44.entities.PGR.update(editing, form);
    else await base44.entities.PGR.create(form);
    setSaving(false);
    setModalOpen(false);
    load();
  };
  ...
  const remove = async (id) => {
    if (!confirm("Excluir este PGR?")) return;
    await base44.entities.PGR.delete(id);
    load();
  };
    ...
    <div className="space-y-4">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {r.grau_risco && <Badge className="text-xs bg-purple-100 text-purple-700">Grau {r.grau_risco}</Badge>}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 mt-2 text-xs text-gray-500">
                      ...
                      {r.data_elaboracao && <span>📅 Elaborado: {format(new Date(r.data_elaboracao), "dd/MM/yyyy")}</span>}
                      {r.data_validade && (
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Editar PGR" : "Novo PGR"}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-3 py-2">
            {[
              ["Título *", "titulo", "text"],
              ["Empresa *", "empresa_nome", "text"],
              ["Unidade/Filial", "unidade", "text"],
              ["Responsável Técnico", "responsavel_tecnico", "text"],
              ["CREA/CAU", "crea_cau", "text"],
              ["Versão", "versao", "text"],
              ["Data Elaboração", "data_elaboracao", "date"],
              ["Data Revisão", "data_revisao", "date"],
            ...
            <div>
              <label className="text-xs font-medium text-gray-600 block mb-1">Grau de Risco (1-4)</label>
              <Input type="number" min={1} max={4} value={form.grau_risco || 2} onChange={e => setForm(f => ({ ...f, grau_risco: Number(e.target.value) }))} />
            </div>
            <div>
