...
const EMPTY_FORM = {
  ...
  data_emissao: "", data_validade: "", status: "Rascunho", observacoes: ""
};
...
export default function ASOTab() {
  ...
  const handleFileUpload = async (e) => {
    ...
    const record = await base44.entities.DocumentoASA.create({
    ...
    try {
      const result = await base44.integrations.Core.InvokeLLM({
        prompt: `Analise este documento ASA (Atestado de Saúde Ambiental) e extraia em JSON: empresa_nome, cnpj, medico_responsavel, crm, data_emissao (DD/MM/AAAA), data_validade (DD/MM/AAAA). Retorne APENAS o JSON.`,
        file_urls: [file_url],
        response_json_schema: {
          ...
          properties: {
            ...
            data_emissao: { type: "string" }, data_validade: { type: "string" }
          }
        }
      });
      const extracted = {
        ...
        crm: result.crm || "",
        data_emissao: parseBRDate(result.data_emissao),
        data_validade: parseBRDate(result.data_validade),
        status: "Ativo", observacoes: "", extraido_por_ia: true
      };
      await base44.entities.DocumentoASA.update(record.id, extracted);
      setForm(extracted);
  ...
  const save = async () => {
    setSaving(true);
    if (currentId) await base44.entities.DocumentoASA.update(currentId, form);
    setSaving(false);
    setModalOpen(false);
    load();
  };
  ...
  const cancel = async () => {
    if (step === 2 && currentId && !editingId) {
      await base44.entities.DocumentoASA.delete(currentId);
    }
    setModalOpen(false);
  };

  const remove = async (id) => {
    if (!confirm("Excluir esta ASA?")) return;
    await base44.entities.DocumentoASA.delete(id);
    load();
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <Button onClick={openNew} className="gap-1.5 bg-rose-600 hover:bg-rose-700">
          <Plus className="w-4 h-4" /> Nova ASA
        </Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-gray-50 rounded-xl border">
          ...
          <p className="text-gray-500 font-medium">Nenhuma ASA cadastrada</p>
          <p className="text-gray-400 text-sm mt-1">Clique em "+ Nova ASA" para começar</p>
        </div>
      ) : (
                ...
                return (
                  <tr key={r.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${vencido ? "bg-red-50/50" : ""}`}>
                    ...
                    <td className={`px-4 py-3 font-medium ${vencido ? "text-red-600" : "text-gray-700"}`}>
                      {r.data_validade
                        ? <span>{vencido ? "⚠ " : ""}{format(new Date(r.data_validade + "T00:00:00"), "dd/MM/yyyy")}</span>
      ...
      <Dialog open={modalOpen} onOpenChange={(open) => { if (!open) cancel(); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{step === 1 ? "Nova ASA — Upload do Documento" : editingId ? "Editar ASA" : "Identificar ASA"}</DialogTitle>
          </DialogHeader>
            ...
            <div className="py-6 flex flex-col items-center gap-4">
              <div className="w-full border-2 border-dashed border-gray-300 rounded-xl p-10 flex flex-col items-center gap-3 cursor-pointer hover:border-rose-400 hover:bg-rose-50/30 transition-colors"
                ...
                {uploading ? (
                  ...
                  <><Upload className="w-10 h-10 text-gray-400" /><p className="font-medium text-gray-700">Selecione o arquivo ASA em PDF</p><p className="text-xs text-gray-400">Clique aqui ou arraste o arquivo</p></>
                )}
              </div>
            ...
            <>
              ...
              <div className="grid gap-3 py-1">
                {[
                  ...
                  ["CRM", "crm", "text"],
                  ["Data de Emissão", "data_emissao", "date"],
