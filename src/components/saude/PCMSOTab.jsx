...
import { Plus, Search, Activity, Edit2, Trash2 } from "lucide-react";
...
const EMPTY = {
  titulo: "", empresa_nome: "", unidade: "", medico_coordenador: "",
  crm: "", data_elaboracao: "", data_validade: "", ano_vigencia: new Date().getFullYear(),
  status: "Elaboração", total_funcionarios: 0, observacoes: ""
};
...
export default function PCMSOTab() {
  ...
  const load = async () => {
    setLoading(true);
    const data = await base44.entities.PCMSO.list("-created_date", 100);
    setRecords(data);
    setLoading(false);
  };
  ...
  const save = async () => {
    setSaving(true);
    if (editing) await base44.entities.PCMSO.update(editing, form);
    else await base44.entities.PCMSO.create(form);
    setSaving(false);
    setModalOpen(false);
    load();
  };
  ...
  const remove = async (id) => {
    ...
    await base44.entities.PCMSO.delete(id);
    load();
  };
    ...
    <div className="space-y-4">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {r.ano_vigencia && <Badge className="text-xs bg-blue-100 text-blue-700">{r.ano_vigencia}</Badge>}
                    </div>
                    <p className="text-sm text-gray-600 mt-1">{r.empresa_nome}{r.unidade ? ` — ${r.unidade}` : ""}</p>
                    <div className="flex flex-wrap gap-4 mt-2 text-xs text-gray-500">
                      {r.medico_coordenador && <span>👨‍⚕️ Dr. {r.medico_coordenador}</span>}
                      {r.crm && <span>CRM {r.crm}</span>}
                      {r.total_funcionarios > 0 && <span>👥 {r.total_funcionarios} func.</span>}
                      {r.data_validade && (
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Editar PCMSO" : "Novo PCMSO"}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-3 py-2">
            {[
              ...
              ["Médico Coordenador", "medico_coordenador", "text"],
              ["CRM", "crm", "text"],
              ["Ano de Vigência", "ano_vigencia", "number"],
              ["Total de Funcionários", "total_funcionarios", "number"],
