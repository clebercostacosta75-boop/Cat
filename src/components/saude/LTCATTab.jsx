...
export default function LTCATTab({ empresa, onEmpresaAtualizada }) {
  ...
  const load = async () => {
    setLoading(true);
    const data = await base44.entities.DocumentoLTCAT.filter({ cnpj: empresa.cnpj });
  ...
  const handleFileUpload = async (e) => {
    ...
    const record = await base44.entities.DocumentoLTCAT.create({
    ...
    try {
      const result = await base44.integrations.Core.InvokeLLM({
        prompt: `Analise este documento LTCAT (Laudo Técnico das Condições Ambientais do Trabalho) e extraia em JSON: empresa_nome, cnpj, responsavel_tecnico, registro_profissional, data_elaboracao (DD/MM/AAAA), vigencia_inicio (DD/MM/AAAA), vigencia_fim (DD/MM/AAAA). Retorne APENAS o JSON.`,
        file_urls: [file_url],
        response_json_schema: {
          type: "object",
          properties: {
            empresa_nome: { type: "string" }, cnpj: { type: "string" },
            responsavel_tecnico: { type: "string" }, registro_profissional: { type: "string" },
      ...
      const extracted = {
        ...
        registro_profissional: result.registro_profissional || "",
      ...
      await base44.entities.DocumentoLTCAT.update(record.id, extracted);
      setForm(extracted);
  ...
  const save = async () => {
    setSaving(true);
    if (currentId) await base44.entities.DocumentoLTCAT.update(currentId, { ...form, empresa_mestre_id: empresa.id });
    setSaving(false);
    setModalOpen(false);
    load();
  };
  ...
  const cancel = async () => {
    if (step === 2 && currentId && !editingId) {
      await base44.entities.DocumentoLTCAT.delete(currentId);
    }
    setModalOpen(false);
  };
  ...
  const remove = async (id) => {
    ...
    await base44.entities.DocumentoLTCAT.delete(id);
    load();
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <Button onClick={openNew} className="gap-1.5 bg-rose-600 hover:bg-rose-700">
          <Plus className="w-4 h-4" /> Importar LTCAT
        </Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-gray-50 rounded-xl border">
          ...
          <p className="text-gray-500 font-medium">Nenhum LTCAT cadastrado para {empresa.razao_social}</p>
          <p className="text-gray-400 text-sm mt-1">Clique em "Importar LTCAT" para fazer upload do documento</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-600">Registro Profissional</th>
              ...
              {filtered.map(r => {
                ...
                return (
                  <tr key={r.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${vencido ? "bg-red-50/50" : ""}`}>
                    ...
                    <td className="px-4 py-3 text-gray-600">{r.registro_profissional || "—"}</td>
      ...
      <Dialog open={modalOpen} onOpenChange={(open) => { if (!open) cancel(); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{step === 1 ? "Importar LTCAT — Upload do Documento" : editingId ? "Editar LTCAT" : "Identificar LTCAT"}</DialogTitle>
          </DialogHeader>
            ...
            <div className="py-6 flex flex-col items-center gap-4">
              <div className="w-full border-2 border-dashed border-gray-300 rounded-xl p-10 flex flex-col items-center gap-3 cursor-pointer hover:border-rose-400 hover:bg-rose-50/30 transition-colors"
                ...
                {uploading ? (
                  ...
                  <p className="font-medium text-gray-700">Selecione o LTCAT em PDF</p>
            ...
            <>
              ...
              <div className="grid gap-3 py-1">
                {[
                  ...
                  ["Registro Profissional (CREA/CRO)", "registro_profissional", "text"],
