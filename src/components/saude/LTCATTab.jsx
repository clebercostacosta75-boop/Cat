...
const EMPTY_FORM = {
  empresa_nome: "", cnpj: "", responsavel_tecnico: "", registro_profissional: "",
...
export default function LTCATTab() {
  ...
  const load = async () => {
    setLoading(true);
    const data = await base44.entities.DocumentoLTCAT.list("-created_date", 100);
    setRecords(data);
    setLoading(false);
  };
  ...
  const save = async () => {
    setSaving(true);
    if (currentId) await base44.entities.DocumentoLTCAT.update(currentId, form);
    setSaving(false);
    setModalOpen(false);
    load();
  };
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-gray-50 rounded-xl border">
          ...
          <p className="text-gray-400 text-sm mt-1">Clique em "+ Novo LTCAT" para começar</p>
        </div>
      ) : (
              ...
              {filtered.map(r => {
                ...
                return (
                  <tr key={r.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${vencido ? "bg-red-50/50" : ""}`}>
                    ...
                    <td className="px-4 py-3 text-gray-600">{r.responsavel_tecnico || "—"}</td>
      ...
      <Dialog open={modalOpen} onOpenChange={(open) => { if (!open) cancel(); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{step === 1 ? "Novo LTCAT — Upload do Documento" : editingId ? "Editar LTCAT" : "Identificar LTCAT"}</DialogTitle>
          </DialogHeader>
            ...
            <div className="py-6 flex flex-col items-center gap-4">
              <div className="w-full border-2 border-dashed border-gray-300 rounded-xl p-10 flex flex-col items-center gap-3 cursor-pointer hover:border-rose-400 hover:bg-rose-50/30 transition-colors"
                ...
                {uploading ? (
                  ...
                  <><Upload className="w-10 h-10 text-gray-400" /><p className="font-medium text-gray-700">Selecione o arquivo LTCAT em PDF</p><p className="text-xs text-gray-400">Clique aqui ou arraste o arquivo</p></>
                )}
              </div>
